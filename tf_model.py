...
def trainProtein(numFiles, batch_Size, proteinList, numTraining):
  for currIt in range(0, (len(proteinList)*int(numTraining[0]))):
    ...
    if (int(currIt)%batch_Size == 0):
      a = np.zeros((batch_Size,N,P,M,1),dtype = float)
      b = np.zeros((batch_Size,N*M*P,3), dtype = float)
      c = np.zeros((batch_Size,N,P,M,3), dtype = float)
    ...
    with open(path + str(randNum2) + ".txt") as inf:
      for line in inf:
        ...
        thresh = float(thresh)
        label = int(label)	
        a[int(currIt)%batch_Size][int(it/(32*7))][int(it/32)%7][int(it%32)][0] = thresh
...
def runTrainingBatch(a, b, c):
  ...
  temp = sess.run(modelResult, feed_dict={x_image: a, x_image_2: c})
